...
from sqlalchemy import DateTime, ForeignKey, create_engine, Integer, String, Boolean, select
...
class Song(Base):
    ...
    # relacion ManyToOne con Artist unidireccional
...
def init_db():
    ...
    try:
        ...
        default_artists = [
            ...
            Artist(name="Abaral", birth_date=datetime(1972,8,4)),
...
def find_by_id(id: int, db: Session = Depends(get_db)):
    ...
    song = db.execute(
        select(Song).where(Song.id == id).options(joinedload(Song.artist))).unique().scalar_one_or_none()
